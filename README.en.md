...
- **Tags** — add or remove tags (up to 10). Each tag gets a color used for both the file name and its badge. The first entry ("归档") is the archive tag and cannot be renamed.
- **Navigation icons** — toggle file and folder icons on or off.

## Bundled CSS Snippet

`snippets/style.css` is a standalone Notion-style CSS snippet, independent of the plugin and compatible with any theme.
